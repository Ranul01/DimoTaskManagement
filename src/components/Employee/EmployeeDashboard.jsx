  ...
  useEffect(() => {
    const checkFirstTimeLogin = async () => {
      ...
      if (userDoc.exists()) {
        ...
        setUserFirstTimeLogin(userData.firstTimeLogin || false);

        // Automatically show password change modal for first-time users
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {showPasswordModal && (
          <ChangePasswordModal
            onClose={() => {
              // Only allow closing if not first-time login
              if (!userFirstTimeLogin) {
                setShowPasswordModal(false);
              }
            }}
            isFirstTime={userFirstTimeLogin}
          />
        )}
      </div>
    </div>
